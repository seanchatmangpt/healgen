...
ENV VIRTUAL_ENV /opt/healgen-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/healgen/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/healgen/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/healgen/ && touch src/healgen/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/healgen/
...
# Expose the app.
ENTRYPOINT ["/opt/healgen-env/bin/healgen"]
CMD []
